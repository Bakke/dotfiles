...
sudo cp -rv .theme_temp/${THEME_DIR} $SDDM_THEMES_DIR || (echo -e "\e31;02mFailed to copy theme to $SDDM_THEMES_DIR!\e[0m" && rm -rf .theme_temp && exit 1)
rm -rf .theme_temp

sudo rm "${SDDM_THEMES_DIR}/${THEME_DIR}/theme.conf"
sudo cp "${HOME}/.dotfiles/config/sddm/theme.conf" "${SDDM_THEMES_DIR}/${THEME_DIR}/theme.conf"
sudo cp "${HOME}/.dotfiles/assets/wallpapers/forest.png" "${SDDM_THEMES_DIR}/${THEME_DIR}/forest.png"

sudo sudo echo -e "[Theme]\nCurrent=${THEME_DIR}" > ${SDDM_CONFIG_PATH} || (echo -e "\e[31;02mFailed to write to ${SDDM_CONFIG_PATH}.\e[0m" && exit 1)
